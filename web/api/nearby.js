...
export default async function handler(req, res) {
  try {
    ...
    const radius = Math.min(
      Number(u.searchParams.get("radius")) || 3000,
      20000
    );
    ...
    const lngDelta =
      radius / (111_320 * Math.cos((lat * Math.PI) / 180) || 1e-6);
    ...
    qs.append("lat", `gte.${minLat}`);
    qs.append("lat", `lte.${maxLat}`);
    qs.append("lng", `gte.${minLng}`);
    qs.append("lng", `lte.${maxLng}`);
