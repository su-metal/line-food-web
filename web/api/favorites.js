...
export default async function handler(req, res) {
  try {
    ...
    if (method === "GET") {
      ...
      if (shopId) {
        // === 単一店がお気に入りかどうか ===
        const q = `/rest/v1/favorites?select=shop_id&user_id=eq.${encodeURIComponent(
          uid
        )}&shop_id=eq.${encodeURIComponent(shopId)}&limit=1`;
        ...
        res.end(
          JSON.stringify({
            ok: true,
            isFav: Array.isArray(rows) && rows.length > 0,
          })
        );
        return;
      }
      ...
      const r1 = await sbFetch(
        `/rest/v1/favorites?select=shop_id,created_at&user_id=eq.${encodeURIComponent(
          uid
      ...
      if (ids.length) {
        ...
        const r2 = await sbFetch(`/rest/v1/shops?${params.toString()}`, {
          method: "GET",
        });
