...
function LeafletAdapter() {
  ...
  return {
    ...
    destroy() { map?.remove(); map = null; markers = []; }
  };
}
...
function GoogleAdapter() {
  ...
  return {
    ...
    async init(el, { center = {lat: 35.68, lng: 139.76}, zoom = 14 } = {}) {
      // 動的ロード（Google）
      await loadJs("https://maps.googleapis.com/maps/api/js?key=YOUR_KEY&libraries=marker");
    ...
    addMarkers(items, { onClick } = {}) {
      items.forEach(s => {
        const m = new google.maps.Marker({
          position: {lat: s.lat, lng: s.lng}, map, title: s.title,
        });
